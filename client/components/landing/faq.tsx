...
const FAQS = [
  ...
  },
  {
    ...
    a: "Your OAuth tokens are encrypted at rest with a key-encryption key, every connection uses least-privilege scopes, and all data is tenant-isolated — queries are physically scoped to your user id, so one account can never read another. Nothing leaves your account without an explicit approval, and disconnecting a tool triggers a full purge: emails, embeddings, and tokens are deleted, not just unlinked.",
  },
  {
          ...
          return (
            <div key={item.q}>
              ...
              >
                ...
                <Plus
                  className={cn("h-4 w-4 shrink-0 text-muted transition-transform", isOpen && "rotate-45 text-accent")}
                />
              </button>
